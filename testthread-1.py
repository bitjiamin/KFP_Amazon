...
class TestThread(QtCore.QThread):
    ...
    def test_thread(self):
        ...
        for i in range(load.threadnum):
            # thread_list.append(threading.Thread(target=test_func, args=(3,)))
            thread_list.append(threading.Thread(target=self.test_func))

        for thread in thread_list:
            thread.setDaemon(True)
            # 这个setDaemon默认为False, 非守护线程
            # 表示主线程等所有子线程结束后, 在结束
